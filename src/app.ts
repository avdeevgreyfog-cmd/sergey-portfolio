import { capabilities, publicProjects, type Project } from './data.js';

type Product = {
  id: string;
  name: string;
  category: 'Pendant' | 'Table' | 'Floor';
  price: number;
  material: string;
  image: string;
  imageAlt: string;
...
type DemoState = {
  ...
  crmStatus: Record<string, string>;
};
...
const testPath = (window as Window & { __PORTFOLIO_TEST_PATH__?: string }).__PORTFOLIO_TEST_PATH__;
const configuredBasePath = document.querySelector<HTMLMetaElement>('meta[name="app-base"]')?.content || '';
...
function icon(name: 'arrow'|'menu'|'close'|'heart'|'bag'|'search'|'check'|'play'|'grid'|'filter'|'back'|'plus'|'minus'): string {
...
function shellHeader(active = ''): string {
  return `
...
function renderWork(project: Project) {
...
function demoToolbar(project: Project): string {
...
function renderDemo(project: Project) {
  ...
  let content = '';
  if (project.slug === 'raznye-ludi') content = renderRaznyeDemo();
  if (project.slug === 'b2b-engineering') content = renderB2BDemo();
  if (project.slug === 'design-light-store') content = renderShopDemo();
  if (project.slug === 'r-kadry-demo') content = renderCrmDemo();
  root.innerHTML = `${demoToolbar(project)}<div class="demo-stage" data-demo-stage>${content}</div>`;
  bindDemoToolbar();
  if (project.slug === 'raznye-ludi') bindRaznyeDemo();
  if (project.slug === 'b2b-engineering') bindB2BDemo();
  if (project.slug === 'design-light-store') bindShopDemo();
  if (project.slug === 'r-kadry-demo') bindCrmDemo();
}

function renderRaznyeDemo(): string {
...
function renderShopDemo(): string {
...
function renderCrmDashboard(includeTitle = true): string {
...
function renderCrmTable(rows = crmRequests): string {
...
function bindDemoToolbar() {
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-viewport]').forEach(button => button.addEventListener('click', () => {
...
function bindRaznyeDemo() {
  const hud = document.querySelector<HTMLElement>('[data-rz-hud]');
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-loadout-step]').forEach(btn=>btn.addEventListener('click',()=>{
    document.querySelectorAll('[data-loadout-step]').forEach(b=>b.classList.remove('active')); btn.classList.add('active');
    if (hud) hud.textContent = labels[Number(btn.dataset.loadoutStep)];
  }));
  const form = document.querySelector<HTMLFormElement>('[data-demo-form]');
  form?.addEventListener('submit', async e => {
    e.preventDefault(); const msg = form.querySelector<HTMLElement>('[data-form-message]');
    if (!form.checkValidity()) { form.reportValidity(); if(msg){msg.textContent='Заполните имя и контакт.'; msg.className='form-message error';} return; }
    const submit = form.querySelector<HTMLButtonElement>('button[type=submit]')!; submit.disabled=true; submit.textContent='Отправляем…'; await delay(650); submit.disabled=false; submit.innerHTML=`Готово ${icon('check')}`; if(msg){msg.textContent='Demo-заявка обработана локально. Ничего не отправлено.'; msg.className='form-message success';}
  });
}

function bindB2BDemo() {
  const form = document.querySelector<HTMLFormElement>('[data-b2b-form]');
  const estimate = document.querySelector<HTMLElement>('[data-b2b-estimate]');
  const recalc = () => {
    if (!form || !estimate) return;
    const data = new FormData(form); const area = Math.max(100, Number(data.get('area')) || 100); const scope = String(data.get('scope'));
    const object = String(data.get('object')); const baseRate = object==='plant'?920:object==='retail'?660:540; const factor = scope==='full'?1:scope==='design'?.22:.14;
    const value = Math.round(area*baseRate*factor/10000)*10000; estimate.textContent = `Ориентир: от ${money(value)}`;
  };
  form?.querySelectorAll('select,input[name=area]').forEach(el=>el.addEventListener('input',recalc)); recalc();
  document.querySelectorAll<HTMLButtonElement>('[data-service]').forEach(btn=>btn.addEventListener('click',()=>{ document.querySelector('#brief')?.scrollIntoView({behavior:'smooth'}); const scope=form?.querySelector<HTMLSelectElement>('select[name=scope]'); if(scope){scope.value='full'; recalc();}}));
  form?.addEventListener('submit', async e=>{
    e.preventDefault(); const msg=form.querySelector<HTMLElement>('[data-form-message]');
    if(!form.checkValidity()){form.reportValidity(); if(msg){msg.textContent='Проверьте обязательные поля.';msg.className='form-message error';}return;}
    const email=(form.elements.namedItem('email') as HTMLInputElement).value; const button=form.querySelector<HTMLButtonElement>('button[type=submit]')!; button.disabled=true;button.textContent='Считаем…';await delay(750);button.disabled=false;
    if(email.toLowerCase().includes('error')){button.innerHTML=`Повторить ${icon('arrow')}`;if(msg){msg.textContent='Demo error-state: расчёт временно недоступен. Попробуйте ещё раз.';msg.className='form-message error';}return;}
    button.innerHTML=`Готово ${icon('check')}`;if(msg){msg.textContent=`Demo-расчёт сформирован: ${estimate?.textContent?.replace('Ориентир: ','')}. Заявка не отправлялась.`;msg.className='form-message success';}
  });
}

function cartCount(): number { return Object.values(state.cart).reduce((sum,qty)=>sum+qty,0); }
function cartTotal(): number { return products.reduce((sum,p)=>sum+(state.cart[p.id]||0)*p.price,0); }

function bindShopDemo() {
  bindShopCatalog(); bindShopActions(); bindProductButtons(); bindLightScene(); updateShopCounters();
}

function bindShopCatalog() {
  const grid = document.querySelector<HTMLElement>('[data-product-grid]'); const search = document.querySelector<HTMLInputElement>('[data-catalog-search]'); const sort = document.querySelector<HTMLSelectElement>('[data-sort]'); const empty = document.querySelector<HTMLElement>('[data-catalog-empty]'); let category='all';
  const apply = () => {
    if(!grid) return; const query=(search?.value||'').trim().toLowerCase(); let items=[...products].filter(p=>(category==='all'||p.category===category)&&(!query||p.name.toLowerCase().includes(query)||p.material.toLowerCase().includes(query)));
    if(sort?.value==='price-asc') items.sort((a,b)=>a.price-b.price); if(sort?.value==='price-desc') items.sort((a,b)=>b.price-a.price);
    grid.innerHTML=items.map(renderProductCard).join(''); if(empty) empty.hidden=items.length>0; bindProductButtons();
  };
  search?.addEventListener('input',apply); sort?.addEventListener('change',apply); document.querySelectorAll<HTMLButtonElement>('[data-category-filter]').forEach(btn=>btn.addEventListener('click',()=>{category=btn.dataset.categoryFilter!;document.querySelectorAll('[data-category-filter]').forEach(b=>b.classList.remove('active'));btn.classList.add('active');apply();}));
...
function bindProductButtons() {
  document.querySelectorAll<HTMLButtonElement>('[data-favorite]').forEach(btn=>{btn.onclick=()=>{const id=btn.dataset.favorite!;state.favorites.has(id)?state.favorites.delete(id):state.favorites.add(id);btn.classList.toggle('is-active',state.favorites.has(id));updateShopCounters();};});
  document.querySelectorAll<HTMLButtonElement>('[data-product-open]').forEach(btn=>{btn.onclick=()=>openProduct(btn.dataset.productOpen!);});
}

function updateShopCounters(){document.querySelectorAll<HTMLElement>('[data-fav-count]').forEach(el=>el.textContent=String(state.favorites.size));document.querySelectorAll<HTMLElement>('[data-cart-count]').forEach(el=>el.textContent=String(cartCount()));}

function openProduct(id:string){const p=products.find(x=>x.id===id);const modal=document.querySelector<HTMLElement>('[data-product-modal]');const body=document.querySelector<HTMLElement>('[data-product-dialog-body]');if(!p||!modal||!body)return;body.innerHTML=`<div class="product-detail-media"><img src="${p.image}" alt="${escapeHtml(p.imageAlt)}"></div><div class="product-detail-copy"><span>${p.category} / Demo product</span><h2>${p.name}</h2><p>${p.description}</p><div class="product-price">${money(p.price)}</div><fieldset><legend>Цвет</legend>${p.colors.map((c,i)=>`<label><input type="radio" name="color" value="${c}" ${i===0?'checked':''}><span>${c}</span></label>`).join('')}</fieldset><button type="button" data-add-cart="${p.id}">Добавить в корзину ${icon('bag')}</button><small>Товар и цена демонстрационные.</small></div>`;modal.hidden=false;document.body.classList.add('modal-open');body.querySelector<HTMLButtonElement>('[data-add-cart]')?.addEventListener('click',()=>{state.cart[p.id]=(state.cart[p.id]||0)+1;updateShopCounters();body.querySelector<HTMLButtonElement>('[data-add-cart]')!.innerHTML=`Добавлено ${icon('check')}`;});document.querySelectorAll('[data-product-close]').forEach(el=>(el as HTMLElement).onclick=()=>closeProduct());}
function closeProduct(){const modal=document.querySelector<HTMLElement>('[data-product-modal]');if(modal)modal.hidden=true;document.body.classList.remove('modal-open');}

function bindShopActions(){document.querySelector('[data-shop-cart]')?.addEventListener('click',()=>openDrawer('cart'));document.querySelector('[data-shop-favorites]')?.addEventListener('click',()=>openDrawer('favorites'));document.querySelector('[data-shop-search]')?.addEventListener('click',()=>{document.querySelector('#catalog')?.scrollIntoView({behavior:'smooth'});setTimeout(()=>document.querySelector<HTMLInputElement>('[data-catalog-search]')?.focus(),500);});document.querySelectorAll('[data-drawer-close]').forEach(el=>(el as HTMLElement).onclick=()=>closeDrawer());}

function openDrawer(mode:'cart'|'favorites'){const drawer=document.querySelector<HTMLElement>('[data-shop-drawer]');const title=document.querySelector<HTMLElement>('[data-drawer-title]');const body=document.querySelector<HTMLElement>('[data-drawer-body]');if(!drawer||!body||!title)return;title.textContent=mode==='cart'?'Корзина':'Избранное';drawer.hidden=false;document.body.classList.add('modal-open');if(mode==='favorites'){const fav=products.filter(p=>state.favorites.has(p.id));body.innerHTML=fav.length?`<div class="drawer-products">${fav.map(p=>`<article><img src="${p.image}" alt=""><div><b>${p.name}</b><span>${money(p.price)}</span><button type="button" data-product-open="${p.id}">Открыть</button></div></article>`).join('')}</div>`:`<div class="drawer-empty">В избранном пока ничего нет.</div>`;bindProductButtons();return;}renderCartDrawer(body);}
function renderCartDrawer(body:HTMLElement){const entries=products.filter(p=>state.cart[p.id]);body.innerHTML=entries.length?`<div class="drawer-products">${entries.map(p=>`<article><img src="${p.image}" alt=""><div><b>${p.name}</b><span>${money(p.price)} · ${state.cart[p.id]} шт.</span><div class="qty"><button type="button" data-qty="minus" data-id="${p.id}">${icon('minus')}</button><strong>${state.cart[p.id]}</strong><button type="button" data-qty="plus" data-id="${p.id}">${icon('plus')}</button></div></div></article>`).join('')}</div><div class="cart-total"><span>Итого</span><strong>${money(cartTotal())}</strong><button type="button" data-checkout>Перейти к checkout ${icon('arrow')}</button><small>Оплата не подключена.</small></div>`:`<div class="drawer-empty">Корзина пуста.</div>`;body.querySelectorAll<HTMLButtonElement>('[data-qty]').forEach(btn=>btn.onclick=()=>{const id=btn.dataset.id!;if(btn.dataset.qty==='plus')state.cart[id]=(state.cart[id]||0)+1;else state.cart[id]=Math.max(0,(state.cart[id]||0)-1);updateShopCounters();renderCartDrawer(body);});body.querySelector<HTMLButtonElement>('[data-checkout]')?.addEventListener('click',()=>renderCheckout(body));}
function renderCheckout(body:HTMLElement){body.innerHTML=`<form class="checkout-form" data-checkout-form novalidate><div class="checkout-step">Checkout / Demo</div><h2>Данные для доставки</h2><label>Имя<input name="name" required autocomplete="name"></label><label>Email<input name="email" type="email" required autocomplete="email"></label><label>Город<input name="city" required></label><label>Адрес<input name="address" required autocomplete="street-address"></label><div class="checkout-summary"><span>Сумма</span><strong>${money(cartTotal())}</strong></div><button type="submit">Подтвердить demo-заказ ${icon('arrow')}</button><p class="form-message" data-form-message aria-live="polite"></p><small>Платёжные данные не запрашиваются. Транзакция не выполняется.</small></form>`;const form=body.querySelector<HTMLFormElement>('[data-checkout-form]')!;form.addEventListener('submit',async e=>{e.preventDefault();const msg=form.querySelector<HTMLElement>('[data-form-message]');if(!form.checkValidity()){form.reportValidity();if(msg){msg.textContent='Заполните поля доставки.';msg.className='form-message error';}return;}const btn=form.querySelector<HTMLButtonElement>('button[type=submit]')!;btn.disabled=true;btn.textContent='Создаём demo-заказ…';await delay(800);state.cart={};updateShopCounters();body.innerHTML=`<div class="checkout-success">${icon('check')}<span>Demo order complete</span><h2>Заказ создан локально.</h2><p>Никаких платежей и реальных заявок не было. Корзина очищена, сценарий завершён.</p><button type="button" data-drawer-close-finish>Вернуться в каталог</button></div>`;body.querySelector('[data-drawer-close-finish]')?.addEventListener('click',()=>{closeDrawer();document.querySelector('#catalog')?.scrollIntoView({behavior:'smooth'});});});}
function closeDrawer(){const drawer=document.querySelector<HTMLElement>('[data-shop-drawer]');if(drawer)drawer.hidden=true;document.body.classList.remove('modal-open');}

function bindLightScene(){const scene=document.querySelector<HTMLElement>('[data-light-scene]');document.querySelectorAll<HTMLButtonElement>('[data-light]').forEach(btn=>btn.addEventListener('click',()=>{document.querySelectorAll('[data-light]').forEach(b=>b.classList.remove('active'));btn.classList.add('active');scene?.setAttribute('data-mode',btn.dataset.light!);}));}
...
function bindCrmNavigation(){document.querySelectorAll<HTMLButtonElement>('[data-crm-nav]').forEach(btn=>btn.onclick=()=>renderCrmView(btn.dataset.crmNav!));document.querySelectorAll<HTMLButtonElement>('[data-request-open]').forEach(btn=>btn.onclick=()=>renderRequestDetail(btn.dataset.requestOpen!));}
function activateCrmNav(view:string){document.querySelectorAll('[data-crm-nav]').forEach(b=>b.classList.toggle('active',(b as HTMLElement).dataset.crmNav===view));document.querySelector('.crm-sidebar')?.classList.remove('is-open');}
function renderCrmView(view:string){const content=document.querySelector<HTMLElement>('[data-crm-content]');if(!content)return;activateCrmNav(view);if(view==='dashboard')content.innerHTML=renderCrmDashboard(true);if(view==='clients')content.innerHTML=`<div class="crm-page-head"><div><span>Клиенты</span><h1>Клиентская база</h1></div><button type="button">+ Новый клиент</button></div><div class="crm-client-grid">${[['Север Проект','Москва','2 активные заявки'],['Альта Сервис','Подольск','1 черновик'],['Пром Лайн','Калуга','3 объекта'],['Логика Склад','Домодедово','архив']].map((c,i)=>`<article><span>${String(i+1).padStart(2,'0')}</span><h3>${c[0]}</h3><p>${c[1]}</p><small>${c[2]}</small></article>`).join('')}</div>`;if(view==='requests')content.innerHTML=`<div class="crm-page-head"><div><span>Заявки</span><h1>Рабочие заявки</h1></div><div class="crm-filter"><input data-crm-filter placeholder="Фильтр по клиенту или роли"><select data-status-filter><option value="all">Все статусы</option>${['Черновик','На согласовании','Согласован','Отказ'].map(x=>`<option>${x}</option>`).join('')}</select></div></div><div data-crm-table-host>${renderCrmTable()}</div>`;if(view==='calculator')content.innerHTML=renderCalculator();bindCrmNavigation();if(view==='requests')bindCrmFilters();if(view==='calculator')bindCalculator();}
function bindCrmFilters(){const input=document.querySelector<HTMLInputElement>('[data-crm-filter]');const select=document.querySelector<HTMLSelectElement>('[data-status-filter]');const host=document.querySelector<HTMLElement>('[data-crm-table-host]');const apply=()=>{const q=(input?.value||'').toLowerCase();const s=select?.value||'all';const rows=crmRequests.filter(r=>(!q||r.client.toLowerCase().includes(q)||r.role.toLowerCase().includes(q))&&(s==='all'||state.crmStatus[r.id]===s));if(host)host.innerHTML=rows.length?renderCrmTable(rows):`<div class="crm-empty">По фильтру ничего не найдено.</div>`;bindCrmNavigation();};input?.addEventListener('input',apply);select?.addEventListener('change',apply);}
function renderRequestDetail(id:string){const r=crmRequests.find(x=>x.id===id);const content=document.querySelector<HTMLElement>('[data-crm-content]');if(!r||!content)return;activateCrmNav('requests');content.innerHTML=`<button class="crm-back" type="button" data-crm-nav="requests">${icon('back')} К заявкам</button><div class="crm-request-head"><div><span>${r.id} · ${r.date}</span><h1>${r.client}</h1><p>${r.role} · ${r.city} · ${r.people} человек</p></div><label>Статус<select data-request-status="${r.id}">${['Черновик','На согласовании','Согласован','Отказ'].map(x=>`<option ${state.crmStatus[r.id]===x?'selected':''}>${x}</option>`).join('')}</select></label></div><div class="crm-request-grid"><section><h2>Условия</h2><dl><div><dt>Ставка сотруднику</dt><dd>${r.wage} ₽/ч</dd></div><div><dt>Численность</dt><dd>${r.people}</dd></div><div><dt>График</dt><dd>6/1 · 11 оплачиваемых часов</dd></div><div><dt>Проживание</dt><dd>Включить в расчёт</dd></div></dl></section><section><h2>Быстрый расчёт</h2><p>Перейдите в калькулятор, чтобы проверить коммерческую ставку для этой заявки.</p><button type="button" data-crm-nav="calculator">Открыть расчёт ${icon('arrow')}</button></section></div>`;content.querySelector<HTMLSelectElement>('[data-request-status]')?.addEventListener('change',e=>{state.crmStatus[r.id]=(e.target as HTMLSelectElement).value;});bindCrmNavigation();}
...
function bindCalculator(){const form=document.querySelector<HTMLFormElement>('[data-crm-calc]');const result=document.querySelector<HTMLElement>('[data-calc-result]');const calc=()=>{if(!form||!result)return;const d=new FormData(form);const wage=Number(d.get('wage'))||0,hours=Number(d.get('hours'))||1,tax=(Number(d.get('tax'))||0)/100,housing=Number(d.get('housing'))||0,margin=(Number(d.get('margin'))||0)/100;const labour=wage*(1+tax);const housingHourly=housing/hours;const cost=labour+housingHourly;const client=margin>=.95?cost:cost/(1-margin);const day=client*hours;result.innerHTML=`<span>Рекомендуемая ставка</span><strong>${Math.round(client)} ₽/ч</strong><p>${money(Math.round(day))} за смену</p><dl><div><dt>Затраты с нагрузкой</dt><dd>${Math.round(labour)} ₽/ч</dd></div><div><dt>Проживание</dt><dd>${Math.round(housingHourly)} ₽/ч</dd></div><div><dt>Целевая маржа</dt><dd>${Math.round(margin*100)}%</dd></div></dl><small>Демо-формула предназначена только для демонстрации интерфейса.</small>`;};form?.querySelectorAll('input').forEach(i=>i.addEventListener('input',calc));calc();}

function bindLab(){const videoRange=document.querySelector<HTMLInputElement>('[data-video-range]');const video=document.querySelector<HTMLElement>('[data-video-scroll]');videoRange?.addEventListener('input',()=>video?.style.setProperty('--progress',videoRange.value));video?.style.setProperty('--progress',videoRange?.value||'30');let story=0;document.querySelector('[data-story-next]')?.addEventListener('click',()=>{story=(story+1)%3;document.querySelectorAll('[data-story-stack] > div').forEach((el,i)=>el.classList.toggle('active',i===story));});document.querySelectorAll('[data-story-stack] > div')[0]?.classList.add('active');const r3=document.querySelector<HTMLInputElement>('[data-3d-range]');const o3=document.querySelector<HTMLElement>('[data-3d-object]');r3?.addEventListener('input',()=>o3?.style.setProperty('--rot',`${r3.value}deg`));const tr=document.querySelector<HTMLInputElement>('[data-type-range]');const kt=document.querySelector<HTMLElement>('[data-kinetic]');tr?.addEventListener('input',()=>kt?.style.setProperty('--move',`${Number(tr.value)-50}px`));const rr=document.querySelector<HTMLInputElement>('[data-reveal-range]');const rd=document.querySelector<HTMLElement>('[data-reveal-demo]');rr?.addEventListener('input',()=>rd?.style.setProperty('--reveal',`${rr.value}%`));const panels=document.querySelectorAll<HTMLElement>('[data-transition-panels] section');document.querySelector('[data-transition-toggle]')?.addEventListener('click',()=>panels.forEach(p=>p.classList.toggle('active')));}

const delay=(ms:number)=>new Promise(resolve=>setTimeout(resolve,ms));

function route(){
  const workMatch=currentPath.match(/^\/work\/([^/]+)\/?$/); const demoMatch=currentPath.match(/^\/demo\/([^/]+)\/?$/);
  if(currentPath==='/works/'||currentPath==='/works') return renderWorks();
  if(currentPath==='/lab/'||currentPath==='/lab') return renderLab();
  if(workMatch){const p=publicProjects.find(x=>x.slug===workMatch[1]);if(p)return renderWork(p);}
  if(demoMatch){const p=publicProjects.find(x=>x.slug===demoMatch[1]);if(p)return renderDemo(p);}
  renderHome();
}

...
with server() as origin,sync_playwright() as p:
 ...
 page.goto(origin+'/',wait_until='domcontentloaded'); expect(page.locator('h1')).to_contain_text('Код,'); expect(page.locator('h1')).to_contain_text('результат'); expect(page.locator('.orb')).to_be_visible(); expect(page.locator('.grid4')).to_be_visible(); assert page.locator('text=Избранная работа').count()==0; no_overflow(page)

...
with server() as origin,sync_playwright() as p:
 ...
 page.goto(origin+'/',wait_until='domcontentloaded'); expect(page.locator('h1')).to_contain_text('САЙТЫ'); expect(page.locator('h1')).to_contain_text('СИСТЕМЫ'); expect(page.locator('.v8-orbit')).to_be_visible(); expect(page.locator('.v8-serviceList')).to_be_visible(); assert page.locator('text=Избранная работа').count()==0; no_overflow(page)
 ...
 page.goto(origin+'/contact/',wait_until='domcontentloaded'); page.locator('input[name=name]').fill('QA'); page.locator('input[name=contact]').fill('@qa'); page.locator('button[type=submit]').click(); expect(page.locator('[data-contact-result]')).to_be_visible()

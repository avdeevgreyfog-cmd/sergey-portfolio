...
with server() as origin,sync_playwright() as p:
 ...
 for name,route in [('home','/'),('works','/works/'),('raznye','/work/raznye-ludi/'),('ops','/work/operations-os/'),('about','/about/'),('contact','/contact/')]:
  for w,h in [(390,844),(1440,900)]:
   page=b.new_page(viewport={'width':w,'height':h});page.goto(origin+route,wait_until='domcontentloaded');page.evaluate("document.querySelectorAll('[data-reveal]').forEach(e=>e.classList.add('is-visible'))");sw=page.evaluate('document.documentElement.scrollWidth');cw=page.evaluate('document.documentElement.clientWidth')

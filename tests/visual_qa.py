 ...
 with server() as origin,sync_playwright() as p:
  ...
  for name,route in routes:
   for w,h in [(390,844),(1440,900)]:
    page=browser.new_page(viewport={'width':w,'height':h});page.goto(origin+route,wait_until='domcontentloaded');settle(page)
    ...
    if route=='/' and page.get_by_text('Разные люди',exact=True).count():findings.append({'severity':'MAJOR','route':route,'viewport':w,'issue':'project leaked onto personal homepage'})
    if route=='/works/' and page.locator('.v6-project__media img[src*="raznye-ludi-ui.webp"]').count()!=1:findings.append({'severity':'MAJOR','route':route,'viewport':w,'issue':'project UI proof missing'})
    path=OUT/f'{name}-{w}.png';page.screenshot(path=str(path),full_page=True);shots.append(str(path.relative_to(ROOT)));page.close()
  demo=browser.new_page(viewport={'width':1440,'height':900});demo.goto(origin+'/demo/raznye-ludi/',wait_until='domcontentloaded');settle(demo)
  ...
  demo=browser.new_page(viewport={'width':390,'height':844});demo.goto(origin+'/demo/raznye-ludi/',wait_until='domcontentloaded');settle(demo);demo.screenshot(path=str(OUT/'demo-ui-mobile-390.png'),full_page=False);shots.append('qa-screens/demo-ui-mobile-390.png');demo.close();browser.close()

...
def main():
 ...
 with server() as origin,sync_playwright() as p:
  ...
  page=browser.new_page(viewport={'width':1440,'height':900});page.goto(origin+'/',wait_until='networkidle');no_overflow(page);media_ok(page);expect(page.locator('h1')).to_contain_text('Сайты и');assert page.locator('a[href$="/effects/"]').count()==0
  page.locator('a[href$="/works/"]').first.click();page.wait_for_load_state('networkidle');expect(page.locator('h1')).to_have_text('Работы');page.locator('a[href$="/work/raznye-ludi/"]').first.click();page.wait_for_load_state('networkidle');expect(page.locator('h1')).to_have_text('Разные люди');page.locator('a[href$="/demo/raznye-ludi/"]').first.click();page.wait_for_load_state('networkidle');expect(page.locator('h1')).to_contain_text('Присоединяйся');assert page.locator('[data-portfolio-safe-demo]').count()==1;checks.append('Flow 1 home → works → case → autonomous demo')
  page.goto(origin+'/',wait_until='networkidle');page.locator('a[href$="/services/"]').first.click();page.wait_for_load_state('networkidle');expect(page.locator('h1')).to_contain_text('Разработка сайтов');page.locator('a[href$="/contact/"]').last.click();page.wait_for_load_state('networkidle');expect(page.locator('h1')).to_contain_text('Обсудить');checks.append('Flow 2 home → services → contact')
  ...
  ctx=browser.new_context(viewport={'width':390,'height':844},reduced_motion='reduce');r=ctx.new_page();r.goto(origin+'/',wait_until='networkidle');assert r.locator('[data-reveal]').first.evaluate("e=>getComputedStyle(e).opacity")=='1';ctx.close();browser.close();checks.append('reduced motion fallback')

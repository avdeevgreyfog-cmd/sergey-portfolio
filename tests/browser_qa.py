...
def main():
 ...
 with server() as origin,sync_playwright() as p:
  ...
  page.goto(origin+'/effects/scroll-story/',wait_until='networkidle'); page.locator('[data-story-step="1"]').scroll_into_view_if_needed(); page.wait_for_timeout(260); assert page.locator('[data-story-stage]').get_attribute('data-scene')=='1'; page.goto(origin+'/effects/kinetic-type/',wait_until='networkidle'); before=page.locator('[data-kinetic-field]').get_attribute('style') or ''; page.locator('[data-kinetic-field]').hover(position={'x':300,'y':300}); page.wait_for_timeout(100); after=page.locator('[data-kinetic-field]').get_attribute('style') or ''; assert after!=before; checks.append('scroll story + kinetic type interactions')
  ctx=browser.new_context(viewport={'width':390,'height':844},reduced_motion='reduce'); r=ctx.new_page(); r.goto(origin+'/',wait_until='networkidle'); assert r.locator('[data-reveal]').first.evaluate("e=>getComputedStyle(e).opacity")=='1'; ctx.close(); checks.append('reduced motion fallback')
